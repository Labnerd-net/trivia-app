...
- 2026-03-25: Any Category option + category tag — all providers now include "Any Category" (id: 'all') as the first category option; when selected, each question shows a small category badge next to the question number
- 2026-03-25: Dark/light mode toggle — added toggle button (☀️/🌙) to Navbar; theme applied via data-theme on <html>; light theme overrides CSS custom properties; no-flash inline script in index.html; preference persisted in localStorage; added 6 new Navbar tests (82 total)
- 2026-03-26: Backlog bug and quality fixes #2 #4 #5 #13 #14 #20 — added missing Category import in Menu.tsx; fixed answer option key collision in Question.tsx; applyTheme now called on initial load in useTheme.ts; removed errorMessage from useFetch dep array via useRef; added console.warn to getProvider fallback; replaced inline style with tq-form-actions CSS class
- 2026-03-26: Fix backlog #9 #21 #8 — replaced partial regex decodeHtml with he library for full named-entity coverage; removed duplicate decodeHtml from download-trivia.mjs; memoized decoded question/answer values in Question.tsx; added 5 new entity tests (87 total)
- 2026-03-30: Fix backlog #3 — added paginationError state to Quiz.tsx; nextQuestions now shows a dismissible error banner with Retry and Dismiss buttons on failure; cancel errors remain silent; added ERROR_NEXT_QUESTIONS constant; added 4 new tests (91 total)
- 2026-03-30: Performance fix backlog #6 #7 — replaced null-cache guard with in-flight promise pattern in makeLocalProvider and makeSnapshotProvider to prevent duplicate fetches on concurrent calls; .catch resets promise on failure so retries work; exported providerGroups from providers.ts to replace per-render Array.from(new Set(...)) in Menu.tsx
- 2026-03-30: Refactor backlog #17 #18 — extracted useProviderCapabilities hook (hasMultipleDifficulties, hasMultipleTypes) replacing inline length checks in Menu.tsx and Quiz.tsx; removed dataValue from CategoryDef and makeLocalProvider, replaced with optional getCategoryFilterValue param; fixed Menu.page.test.tsx mock (missing providerGroups) and submit-test race condition; 95 tests total passing
- 2026-03-30: Refactor backlog #10 — extracted all provider logic from providers.ts into src/api/adapters/ (opentdb.ts, triviaapi.ts, localProviders.ts, snapshotProviders.ts, utils.ts); shuffleArray deduplicated into utils.ts; providers.ts reduced to 22-line registry; all existing exports and 95 tests unchanged
- 2026-04-03: Local provider seen-question tracking — added per-category seen-question tracker (Map<string, Set<CardQuestion>>) to makeLocalProvider closure; getQuestions now excludes already-seen questions per category, resets silently when pool exhausted; all three local providers benefit automatically; 6 new tests (101 total)
- 2026-05-31: Migrate deployment to Cloudflare Workers — removed Dockerfile, nginx.conf, .dockerignore, and docker-publish.yml GitHub Actions workflow; added wrangler.jsonc with SPA routing config; deployment now handled by Cloudflare Workers Builds connected directly to the GitHub repo
